...
void Enemy::Update()
{
	...
	// **プレイヤーにすでに到達していたら何もしない**
	Player* player = (Player*)FindGameObject<Player>();
	if (player && pos_ == player->GetPos()) return;
...
void Enemy::MoveByDijkstra()
{
	Player* player = (Player*)FindGameObject<Player>();
	if (!player) return;

	Point sp = { pos_.x / CHA_WIDTH, pos_.y / CHA_HEIGHT };  // 敵の現在位置
	Point gp = { player->GetPos().x / CHA_WIDTH, player->GetPos().y / CHA_HEIGHT };  // プレイヤーの位置

	// **ダイクストラ法で最短経路を計算**
	Dijkstra(sp, gp);

	// **プレイヤーに到達できない場合は何もしない**
	if (dist[gp.y][gp.x] == INT_MAX) return;

	// **次の移動先を取得**
	Point next = gp;
	while (pre[next.y][next.x].x != sp.x || pre[next.y][next.x].y != sp.y) {
		next = pre[next.y][next.x];  // 出発点に到達するまで前のノードをたどる
	}

	// **移動方向を決定**
	if (next.x > sp.x) forward_ = RIGHT;
	else if (next.x < sp.x) forward_ = LEFT;
	else if (next.y > sp.y) forward_ = DOWN;
	else if (next.y < sp.y) forward_ = UP;
}
